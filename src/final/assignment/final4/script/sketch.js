...
function draw() {
  ...
  for (var i = 0; i < 50; i++) {
    ...
    // Set a dynamic color for each shape based on noise
    fill(
    ...
    // Use size to control the size of each shape
    var shapeSize = sizeValue * 0.5;

    // Vary shape type based on noise
    var shapeType = floor(noise(frameCount * 0.05 + i) * 5);
    ...
    for (var j = 0; j < 360; j += 30) {
      ...
      // Add a pulsating effect to the shapes with vertical oscillation
      ...
      // Draw different shapes based on the shapeType variable
      switch (shapeType) {
        case 0:
          sphere(shapeSize / 2); // Sphere
          break;
        case 1:
          box(shapeSize); // Cube
          break;
        case 2:
          torus(shapeSize / 2, 5); // Torus
          break;
        case 3:
          cone(shapeSize / 2, shapeSize); // Cone
          break;
        case 4:
          ellipsoid(shapeSize, shapeSize / 2, shapeSize / 4); // Ellipsoid
          break;
      }
